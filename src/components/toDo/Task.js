import React from 'react';
import style from './Task.module.css';
import { connect } from 'react-redux';
import { removeTask } from '../../actions/index.js';


const actionCreators = {
    removeTask: removeTask,
};

const Task = ({removeTask, text, updateText}) => {
    const completeIcon = {
        action: undefined,
        icon: (
            (
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check-circle-fill" viewBox="0 0 16 16">
                    <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zm-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"></path>
                </svg>
          )
        )
    };

    const removeIcon = 
        {
            action: removeTask,
            icon: (
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" className="bi bi-x-circle-fill" viewBox="0 0 16 16" fill="currentColor">
                    <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293 5.354 4.646z"></path>
                </svg>
            )
        };


    const icons = [removeIcon, completeIcon]

    return (
        <li className={style.listItem} >
            <div className="alert alert-primary d-flex flex-row align-items-center justify-content-between" role="alert">
                <div className='p-1'>
                    <p className="m-0">{text}</p>
                </div>
                <div className={`d-flex justify-content-between ${style.buttonsGroup}`}>
                    {icons.map(({ action = () => {}, icon }) => {
                        return <div className={style.taskButton} onClick={action}>{icon}</div>
                    })}
                </div>
            </div>
            
        </li>
    )
}

export default connect()(Task);